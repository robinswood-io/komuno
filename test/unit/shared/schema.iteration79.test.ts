...
describe('shared/schema.js iteration 79 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration79-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration79-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration79-force-catch') {
        throw new Error('iteration79 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration79-valid',
      config: '{"theme":"iteration79"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration79-forced',
      config: 'iteration79-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration79-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration79"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration79-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: updateEventSponsorshipSchema accepts null confirmedAt and rejects negative amount', () => {
    const valid = schema.updateEventSponsorshipSchema.safeParse({
      confirmedAt: null,
      amount: 0,
    });

    const invalid = schema.updateEventSponsorshipSchema.safeParse({
      amount: -1,
    });
