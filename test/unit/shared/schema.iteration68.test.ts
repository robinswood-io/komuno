...
describe('shared/schema.js iteration 68 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration68-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration68-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration68-force-catch') {
        throw new Error('iteration68 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration68-valid',
      config: '{"theme":"iteration68"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration68-forced',
      config: 'iteration68-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration68-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration68"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration68-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: insertTrackingAlertSchema defaults severity and rejects invalid alertType', () => {
    const valid = schema.insertTrackingAlertSchema.safeParse({
      entityType: 'member',
      entityId: 'member-1',
      entityEmail: 'member@example.com',
      alertType: 'stale',
      title: 'Suivi requis',
      message: 'Contact absent depuis 60 jours',
    });

    const invalid = schema.insertTrackingAlertSchema.safeParse({
      entityType: 'member',
      entityId: 'member-1',
      entityEmail: 'member@example.com',
      alertType: 'unknown',
    ...
    if (valid.success) {
      expect(valid.data.severity).toBe('medium');
    }
  });
});
