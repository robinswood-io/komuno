...
describe('shared/schema.js iteration 56 - stubborn line proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback tracks current financialCategories.id across reference() calls', () => {
    ...
    const sentinelA = { name: 'iteration56-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration56-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 by executing JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration56-force-catch') {
        throw new Error('iteration56 forced catch');
      }
    ...
    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration56-valid',
      config: '{"theme":"iteration56"}',
    });

    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration56-forced',
      config: 'iteration56-force-catch',
    });

    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration56-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration56"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration56-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: updateFinancialExpenseSchema accepts nullable budgetId and rejects non-uuid budgetId', () => {
    const validNull = schema.updateFinancialExpenseSchema.safeParse({
      budgetId: null,
    });

    const invalidUuid = schema.updateFinancialExpenseSchema.safeParse({
      budgetId: 'budget-123',
    });
