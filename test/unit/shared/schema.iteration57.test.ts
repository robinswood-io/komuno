...
describe('shared/schema.js iteration 57 - stubborn line proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback tracks current financialCategories.id across reference() calls', () => {
    ...
    const sentinelA = { name: 'iteration57-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration57-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 by executing JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration57-force-catch') {
        throw new Error('iteration57 forced catch');
      }
    ...
    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration57-valid',
      config: '{"theme":"iteration57"}',
    });

    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration57-forced',
      config: 'iteration57-force-catch',
    });

    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration57-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration57"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration57-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: updateFinancialForecastSchema accepts nullable notes and rejects unknown confidence', () => {
    const validNotes = schema.updateFinancialForecastSchema.safeParse({
      notes: null,
      basedOn: 'historical',
    });
