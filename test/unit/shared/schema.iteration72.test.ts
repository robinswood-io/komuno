...
describe('shared/schema.js iteration 72 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration72-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration72-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration72-force-catch') {
        throw new Error('iteration72 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration72-valid',
      config: '{"theme":"iteration72"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration72-forced',
      config: 'iteration72-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration72-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration72"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration72-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: tracking metric schema validates metricType enum', () => {
    const valid = schema.insertTrackingMetricSchema.safeParse({
      entityType: 'member',
      entityId: 'member-1',
      entityEmail: 'member@gmail.com',
      metricType: 'engagement',
      metricValue: 42,
    });

    const invalid = schema.insertTrackingMetricSchema.safeParse({
      entityType: 'member',
      entityId: 'member-1',
      entityEmail: 'member@gmail.com',
