...
describe('shared/schema.js iteration 73 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration73-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration73-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration73-force-catch') {
        throw new Error('iteration73 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration73-valid',
      config: '{"theme":"iteration73"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration73-forced',
      config: 'iteration73-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration73-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration73"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration73-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: patron update schema enforces type enum and non-negative duration', () => {
    const valid = schema.insertPatronUpdateSchema.safeParse({
    ...
    const invalidDuration = schema.insertPatronUpdateSchema.safeParse({
      patronId: '3b241101-e2bb-4255-8caf-4136c566a962',
      type: 'meeting',
      subject: 'Point trimestriel',
      date: '2026-05-02',
