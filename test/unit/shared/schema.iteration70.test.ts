...
describe('shared/schema.js iteration 70 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration70-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration70-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration70-force-catch') {
        throw new Error('iteration70 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration70-valid',
      config: '{"theme":"iteration70"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration70-forced',
      config: 'iteration70-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration70-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration70"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration70-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: insertInscriptionSchema validates eventId as UUID', () => {
    const valid = schema.insertInscriptionSchema.safeParse({
      eventId: '3b241101-e2bb-4255-8caf-4136c566a962',
      name: 'Camille Roy',
      email: 'camille.roy@gmail.com',
    });

    const invalid = schema.insertInscriptionSchema.safeParse({
      eventId: 'event-123',
      name: 'Camille Roy',
      email: 'camille.roy@gmail.com',
    });
