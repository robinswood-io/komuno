...
describe('shared/schema.js iteration 53 - stubborn line proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback tracks current financialCategories.id across reference() calls', () => {
    ...
    const sentinelA = { name: 'iteration53-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration53-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 by executing JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration53-force-catch') {
        throw new Error('iteration53 forced catch');
      }
    ...
    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration53-valid',
      config: '{"theme":"iteration53"}',
    });

    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration53-forced',
      config: 'iteration53-force-catch',
    });

    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration53-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration53"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration53-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: feature config defaults enabled=true and enforces key max length', () => {
    const valid = schema.insertFeatureConfigSchema.safeParse({
      featureKey: 'new_dashboard',
    });

    const invalidLongKey = schema.insertFeatureConfigSchema.safeParse({
      featureKey: 'x'.repeat(51),
    });

    expect(valid.success).toBe(true);
    expect(invalidLongKey.success).toBe(false);

    if (valid.success) {
      expect(valid.data.enabled).toBe(true);
    }
  });
});
