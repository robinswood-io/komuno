...
describe('shared/schema.js iteration 71 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration71-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration71-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration71-force-catch') {
        throw new Error('iteration71 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration71-valid',
      config: '{"theme":"iteration71"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration71-forced',
      config: 'iteration71-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration71-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration71"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration71-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: updateEventSchema is partial and enforces maxParticipants minimum', () => {
    const validPartial = schema.updateEventSchema.safeParse({
      maxParticipants: 25,
    });

    const invalidMin = schema.updateEventSchema.safeParse({
      maxParticipants: 0,
    });

    const empty = schema.updateEventSchema.safeParse({});
