...
describe('shared/schema.js iteration 54 - stubborn line proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback tracks current financialCategories.id across reference() calls', () => {
    ...
    const sentinelA = { name: 'iteration54-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration54-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 by executing JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration54-force-catch') {
        throw new Error('iteration54 forced catch');
      }
    ...
    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration54-valid',
      config: '{"theme":"iteration54"}',
    });

    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration54-forced',
      config: 'iteration54-force-catch',
    });

    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration54-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration54"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration54-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: updateFinancialCategorySchema is partial and rejects invalid type', () => {
    const partialValid = schema.updateFinancialCategorySchema.safeParse({
      description: null,
    });

    const invalidType = schema.updateFinancialCategorySchema.safeParse({
      type: 'asset',
    });

    const empty = schema.updateFinancialCategorySchema.safeParse({});
