...
describe('shared/schema.js iteration 74 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration74-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration74-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration74-force-catch') {
        throw new Error('iteration74 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration74-valid',
      config: '{"theme":"iteration74"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration74-forced',
      config: 'iteration74-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration74-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration74"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration74-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: updatePatronUpdateSchema validates HH:MM startTime when provided', () => {
