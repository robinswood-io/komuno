...
describe('shared/schema.js iteration 62 - stubborn proofs + useful behavior', () => {
  ...
  it('proves line 1538 callback uses current financialCategories.id on each reference() call', () => {
    ...
    const sentinelA = { name: 'iteration62-id-a' } as unknown as typeof originalId;
    const sentinelB = { name: 'iteration62-id-b' } as unknown as typeof originalId;
  ...
  it('proves lines 1600-1605 through JSON.parse success and catch branches', () => {
    ...
    const parseSpy = vi.spyOn(JSON, 'parse').mockImplementation((text: string) => {
      if (text === 'iteration62-force-catch') {
        throw new Error('iteration62 forced catch');
      }
      return nativeParse(text);
    });

    const valid = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration62-valid',
      config: '{"theme":"iteration62"}',
    });
    const forced = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration62-forced',
      config: 'iteration62-force-catch',
    });
    const malformed = schema.insertBrandingConfigSchema.safeParse({
      key: 'iteration62-malformed',
      config: '{"theme":}',
    });
    ...
    expect(parseSpy).toHaveBeenCalledWith('{"theme":"iteration62"}');
    expect(parseSpy).toHaveBeenCalledWith('iteration62-force-catch');
    expect(parseSpy).toHaveBeenCalledWith('{"theme":}');
  });

  it('adds useful behavior: updateEventStatusSchema allows published and rejects archived', () => {
    const valid = schema.updateEventStatusSchema.safeParse({ status: 'published' });
    const invalid = schema.updateEventStatusSchema.safeParse({ status: 'archived' });
