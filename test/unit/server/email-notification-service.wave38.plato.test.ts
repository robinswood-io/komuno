...
type EmailNotificationServiceLike = { notifyNewIdea: (idea: { title: string }) => Promise<ServiceResult> };
...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => ({ success: true, data: [{ isActive: true, status: 'active', email: 'idea38@example.com' }] })),
  ...
  setCjsModule(emailTemplatesPath, {
    createNewIdeaEmailTemplate: vi.fn(() => {
      throw new Error('idea-template-failure-wave38');
    }),
...
describe('email-notification-service wave38 plato', () => {
  it('notifyNewIdea catches template exception and returns wrapped idea error', async () => {
    setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewIdea({ title: 'Idea 38' });

    expect(result.success).toBe(false);
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('Erreur notification idée');
      expect(result.error.message).toContain('idea-template-failure-wave38');
    } else {
