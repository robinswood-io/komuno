...
function setupDependencies(): { storageMock: StorageMock } {
  const adminFailure = new Error('test-config-admin-failure-wave36');
...
describe('email-notification-service wave36 plato', () => {
  it('testEmailConfiguration returns admin fetch failure before template and email send', async () => {
    ...
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('test-config-admin-failure-wave36');
    } else {
