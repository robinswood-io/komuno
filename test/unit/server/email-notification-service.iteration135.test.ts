...
describe('server/email-notification-service.js iteration135', () => {
  it('short-circuits member proposal with getAdminEmails formatted error when fallback admin normalization crashes', async () => {
    const { templatesMock, emailServiceMock } = setupDependencies({
      recruitmentResult: { success: true, data: null },
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: null as unknown as string }],
      },
    });
    ...
    const result = await emailNotificationService.notifyNewMemberProposal({
      firstName: 'Ada',
      lastName: 'Lovelace',
      email: 'ada135@example.com',
    });

    expect(result.success).toBe(false);
    expect(result.error).toBeInstanceOf(Error);
    expect((result.error as Error).message).toContain('Erreur récupération admins');
    expect(templatesMock.createNewMemberProposalEmailTemplate).not.toHaveBeenCalled();
