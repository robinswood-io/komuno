...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => {
      throw new Error('admins-throw-wave28');
    }),
...
describe('email-notification-service wave28 plato', () => {
  it('testEmailConfiguration returns wrapped admin-fetch error when force refresh throws', async () => {
    setupDependencies();
    ...
    if (result.error instanceof Error) {
      ...
      expect(result.error.message).toContain('admins-throw-wave28');
    } else {
