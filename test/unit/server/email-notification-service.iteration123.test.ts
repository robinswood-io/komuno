...
describe('server/email-notification-service.js iteration123', () => {
  it('rejects when notifyNewEvent sendEmail throws unexpectedly', async () => {
    setupDependencies({ sendEmailThrows: new Error('smtp-throw-event') });
    const { emailNotificationService } = loadServiceModule();

    await expect(emailNotificationService.notifyNewEvent({ title: 'Event 123' }, 'Organizer 123')).rejects.toThrow(
      'smtp-throw-event',
    );
  });
});
