...
describe('server/email-notification-service.js iteration122', () => {
  it('rejects when notifyNewIdea sendEmail throws unexpectedly', async () => {
    setupDependencies({ sendEmailThrows: new Error('smtp-throw-idea') });
    const { emailNotificationService } = loadServiceModule();

    await expect(emailNotificationService.notifyNewIdea({ title: 'Idea 122', proposedBy: 'Nina' })).rejects.toThrow(
      'smtp-throw-idea',
    );
  });
});
