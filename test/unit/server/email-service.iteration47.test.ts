...
describe('server/email-service.js (iteration 47)', () => {
  beforeEach(() => {
  ...
  it('retourne accepted/rejected convertis en chaînes', async () => {
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';
    ...
    sendMailMock.mockResolvedValueOnce({
      messageId: 'msg-42',
      accepted: [{ toString: () => 'accepted-as-object' }],
      rejected: [{ toString: () => 'rejected-as-object' }],
      response: 'queued',
    });
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      const payload = result.data as { accepted: string[]; rejected: string[] };
      expect(payload.accepted).toEqual(['accepted-as-object']);
      expect(payload.rejected).toEqual(['rejected-as-object']);
    }
  });
});
