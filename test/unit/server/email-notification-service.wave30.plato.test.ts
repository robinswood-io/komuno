...
function setupDependencies(): { storageMock: StorageMock } {
  const adminFailure = new Error('loan-admin-fetch-failure-wave30');
  const storageMock: StorageMock = {
    getBrandingConfig: vi.fn(async () => ({ success: true, data: {} })),
    getAllAdmins: vi.fn(async () => ({ success: false, error: adminFailure })),
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewLoanItemEmailTemplate: vi.fn(() => ({ subject: 'loan-30', html: '<p>loan-30</p>' })),
...
describe('email-notification-service wave30 plato', () => {
  it('notifyNewLoanItem short-circuits and returns admin failure object when recipients cannot be loaded', async () => {
    const { storageMock } = setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewLoanItem({ title: 'Video projecteur' });

    expect(storageMock.getAllAdmins).toHaveBeenCalledTimes(1);
    expect(result.success).toBe(false);
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('loan-admin-fetch-failure-wave30');
    } else {
