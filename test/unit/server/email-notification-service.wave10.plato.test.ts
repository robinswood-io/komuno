...
function setupDependencies(): { storageMock: StorageMock; emailServiceMock: EmailServiceMock } {
  ...
  const emailServiceMock: EmailServiceMock = {
    sendEmail: vi.fn(async () => ({ success: true, data: { messageId: 'msg-wave10' } })),
  };
  ...
  const templatesMock: TemplatesMock = {
    ...
    createNewEventEmailTemplate: vi.fn(() => {
      throw new Error('template-event-failure-wave10');
    }),
...
describe('server/email-notification-service.js wave10 plato', () => {
  it('notifyNewEvent catches template failure and returns wrapped event error', async () => {
    ...
    if (result.error instanceof Error) {
      ...
      expect(result.error.message).toContain('template-event-failure-wave10');
    } else {
