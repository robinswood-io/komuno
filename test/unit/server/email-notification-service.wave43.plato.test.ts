...
type EmailNotificationServiceLike = { notifyNewLoanItem: (loanItem: { title: string }) => Promise<ServiceResult> };
...
function setupDependencies(): { storageMock: StorageMock } {
  const adminError = new Error('loan-admin-error-wave43');
...
describe('email-notification-service wave43 plato', () => {
  it('notifyNewLoanItem returns admin lookup error before template/render path', async () => {
    const { storageMock } = setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewLoanItem({ title: 'Wave43 loan' });

    expect(storageMock.getAllAdmins).toHaveBeenCalledTimes(1);
    expect(result.success).toBe(false);
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('loan-admin-error-wave43');
    } else {
