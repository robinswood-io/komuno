...
describe('server/email-notification-service.js iteration140', () => {
  it('stringifies non-Error template crash in notifyNewLoanItem catch branch', async () => {
    const { templatesMock } = setupDependencies();
    templatesMock.createNewLoanItemEmailTemplate.mockImplementation(() => {
      throw 'loan-template-crash-140';
    });
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewLoanItem({ title: 'Loan 140' });

    expect(result.success).toBe(false);
    expect(result.error).toBeInstanceOf(Error);
    expect((result.error as Error).message).toContain('loan-template-crash-140');
  });
});
