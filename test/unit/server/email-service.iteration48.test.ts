...
describe('server/email-service.js (iteration 48)', () => {
  beforeEach(() => {
  ...
  it('journalise la source "base de données" quand config DB disponible', async () => {
    const emailService = loadEmailService();
    const storage = makeStorage({
      success: true,
      data: {
    ...
    expect(loggerMock.info).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ source: 'base de données' }),
    );
  });
});
