...
function loadDbModule(): { module: DbJsModule; neonPool: MockPool } {
  ...
  const stats: PoolStats = { totalCount: 8, idleCount: 2, waitingCount: 0 };
...
describe('server/db.js iteration125 neon remove listener callback', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    cleanupHarness();
  });

  it('logs pool remove event in development on neon provider', () => {
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const setup = loadDbModule();

    setup.neonPool.emit('remove');

    expect(consoleLogSpy).toHaveBeenCalledWith('[DB] Connexion fermée (provider: neon, pool: 8)');
  });
});
