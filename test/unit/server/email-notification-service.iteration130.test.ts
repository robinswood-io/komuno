...
describe('server/email-notification-service.js iteration130', () => {
  it('uses cache path when forceRefresh is explicitly false', async () => {
    const { storageMock } = setupDependencies({
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: 'admin130@example.com' }],
      },
    });
    ...
    const second = await emailNotificationService.getAdminEmails({ forceRefresh: false });

    expect(first).toEqual({ success: true, data: ['admin130@example.com'] });
    expect(second).toEqual({ success: true, data: ['admin130@example.com'] });
