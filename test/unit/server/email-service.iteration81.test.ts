...
describe('server/email-service.js (iteration 81)', () => {
  beforeEach(() => {
  ...
  it('en mode __esModule, initialise puis envoie un email sans erreur', async () => {
    nodemailerMockMode = 'esmodule';
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';
  ...
  it('encapsule un rejet Error de sendMail avec son message stringifié', async () => {
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';

    const emailService = loadEmailService();
    await emailService.reloadConfig();
    sendMailMock.mockRejectedValueOnce(new Error('smtp fatal'));
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error?.message).toBe('Erreur envoi email: Error: smtp fatal');
    }
  });
});
