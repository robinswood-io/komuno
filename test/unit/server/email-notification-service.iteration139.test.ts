...
describe('server/email-notification-service.js iteration139', () => {
  it('passes updated context URLs to event template after updateContext call', async () => {
    ...
    emailNotificationService.updateContext({
      ...
      adminDashboardUrl: 'https://updated.example/admin-zone',
    });

    const result = await emailNotificationService.notifyNewEvent({ title: 'Event 139' }, 'Org 139');

    expect(result.success).toBe(true);
    expect(templatesMock.createNewEventEmailTemplate).toHaveBeenCalledWith(
      { title: 'Event 139' },
      'Org 139',
