...
describe('server/email-notification-service.js iteration112', () => {
  it('returns formatted getAdminEmails error when an active admin email is non-string', async () => {
    setupDependencies({
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: null }],
      },
    });
    ...
    expect(String(result.error?.message)).toContain('Erreur récupération admins');
  });
});
