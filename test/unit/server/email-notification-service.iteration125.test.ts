...
describe('server/email-notification-service.js iteration125', () => {
  it('rejects when member-proposal sendEmail throws on fallback-admin path', async () => {
    setupDependencies({
      recruitmentResult: { success: true, data: { email: null } },
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: 'admin125@example.com' }],
      },
      sendEmailThrows: new Error('smtp-throw-member-fallback'),
    });
    const { emailNotificationService } = loadServiceModule();

    await expect(
      emailNotificationService.notifyNewMemberProposal({
        firstName: 'Ada',
        lastName: 'Lovelace',
        email: 'ada@example.com',
      }),
    ).rejects.toThrow('smtp-throw-member-fallback');
  });
});
