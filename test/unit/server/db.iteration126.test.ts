...
function loadDbModule(): { module: DbJsModule; pgPool: MockPool; loggerMock: LoggerMock } {
  cleanupHarness();
  process.env.DATABASE_URL = 'postgresql://user:pass@localhost:5432/appdb';
  process.env.NODE_ENV = 'production';

  const stats: PoolStats = { totalCount: 4, idleCount: 1, waitingCount: 2 };
  ...
  return { module, pgPool: createdPgPool as MockPool, loggerMock };
}

describe('server/db.js iteration126 standard error listener callback', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    cleanupHarness();
  });

  it('logs structured error payload when standard pool emits error', () => {
    const setup = loadDbModule();
    const err = new Error('standard-pool-error');

    setup.pgPool.emit('error', err, null);

    expect(setup.loggerMock.error).toHaveBeenCalledWith(
      'CRITICAL: Database pool error',
      expect.objectContaining({
        provider: 'standard',
        message: 'standard-pool-error',
        poolStats: expect.objectContaining({ totalCount: 4, idleCount: 1, waitingCount: 2 }),
      }),
    );
  });
});
