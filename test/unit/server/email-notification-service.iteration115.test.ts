...
describe('server/email-notification-service.js iteration115', () => {
  it('propagates sendEmail failure in recruitment-manager path for member proposal notifications', async () => {
    const { emailServiceMock, storageMock } = setupDependencies({
      recruitmentResult: { success: true, data: { email: 'manager@komuno.test' } },
      sendEmailResult: { success: false, error: new Error('smtp-member-failure') },
    });
