...
describe('server/email-notification-service.js iteration138', () => {
  it('logs failure metadata when testEmailConfiguration sendEmail returns failure', async () => {
    const { loggerMock } = setupDependencies({
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: 'admin138@example.com' }],
      },
      sendEmailResult: { success: false, error: new Error('smtp-fail-138') },
    });
    ...
    expect(loggerMock.error).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ recipient: 'admin138@example.com', error: 'Error: smtp-fail-138' }),
    );
  });
});
