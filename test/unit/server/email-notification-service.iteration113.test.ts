...
describe('server/email-notification-service.js iteration113', () => {
  it('caches an empty active-admin list and reuses it until TTL expires', async () => {
    const { storageMock } = setupDependencies({
      adminsResult: {
        success: true,
        data: [
          { isActive: false, status: 'active', email: 'admin1@example.com' },
          { isActive: true, status: 'inactive', email: 'admin2@example.com' },
        ],
      },
    });
