...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => ({ success: true, data: [{ isActive: true, status: 'active', email: 'admin27@example.com' }] })),
    getMemberByCjdRole: vi.fn(async () => ({ success: true, data: { email: 'manager27@example.com' } })),
  };
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewMemberProposalEmailTemplate: vi.fn(() => {
      throw new Error('member-template-failure-wave27');
    }),
...
describe('email-notification-service wave27 plato', () => {
  it('notifyNewMemberProposal catches template exception and returns wrapped member error', async () => {
    setupDependencies();
    ...
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('Erreur notification proposition membre');
      expect(result.error.message).toContain('member-template-failure-wave27');
    } else {
