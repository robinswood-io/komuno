...
describe('server/email-service.js (iteration 78)', () => {
  beforeEach(() => {
  ...
  it('priorise fromEmail DB sur SMTP_FROM_EMAIL env', async () => {
    process.env.SMTP_FROM_EMAIL = 'env-from@example.com';
    ...
    expect(sendMailMock).toHaveBeenCalledWith(
      expect.objectContaining({
        from: '"DB Name" <db-from@example.com>',
      }),
    );
  });
});
