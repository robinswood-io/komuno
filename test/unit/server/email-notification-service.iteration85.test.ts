...
describe('server/email-notification-service.js iteration85', () => {
  it('propagates sendEmail failure from notifyNewEvent', async () => {
    const { emailServiceMock } = setupDependencies({
      sendEmailResult: { success: false, error: new Error('smtp-down') },
    });
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewEvent({ title: 'Event 85' }, 'Owner 85');
