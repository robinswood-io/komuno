...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => ({ success: true, data: [{ isActive: true, status: 'active', email: 'loan42@example.com' }] })),
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewLoanItemEmailTemplate: vi.fn(() => {
      throw new Error('loan-template-failure-wave42');
    }),
...
describe('email-notification-service wave42 plato', () => {
  it('notifyNewLoanItem catches template throw and returns wrapped material error', async () => {
    setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewLoanItem({ title: 'Wave42 loan' });

    expect(result.success).toBe(false);
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('Erreur notification matériel');
      expect(result.error.message).toContain('loan-template-failure-wave42');
    } else {
