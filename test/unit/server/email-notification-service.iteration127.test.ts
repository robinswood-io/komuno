...
describe('server/email-notification-service.js iteration127', () => {
  it('returns formatted error when testEmailConfiguration sendEmail throws unexpectedly', async () => {
    setupDependencies({
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: 'admin127@example.com' }],
      },
      sendEmailThrows: new Error('smtp-throw-test-config'),
    });
    ...
    expect(String(result.error?.message)).toContain('Erreur test email');
  });
});
