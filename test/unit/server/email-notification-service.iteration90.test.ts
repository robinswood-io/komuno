...
describe('server/email-notification-service.js iteration90', () => {
  it('handles template exception in notifyNewLoanItem without throwing', async () => {
    const { emailServiceMock } = setupDependencies({
      throwLoanTemplate: true,
    });
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewLoanItem({ title: 'Laptop 90' });

    expect(result.success).toBe(false);
    expect(result.error).toBeInstanceOf(Error);
    expect(result.error?.message).toContain('notification matériel');
