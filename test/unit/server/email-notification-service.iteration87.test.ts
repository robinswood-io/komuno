...
describe('server/email-notification-service.js iteration87', () => {
  it('falls back to active admins when recruitment manager email is missing', async () => {
    const { emailServiceMock } = setupDependencies({
      recruitmentResult: { success: true, data: null },
      adminsResult: {
        success: true,
        data: [
          { isActive: true, status: 'active', email: 'admin-a@example.com' },
          { isActive: true, status: 'active', email: 'admin-b@example.com' },
        ],
      },
    });
    ...
    expect(payload.to).toEqual(['admin-a@example.com', 'admin-b@example.com']);
  });
});
