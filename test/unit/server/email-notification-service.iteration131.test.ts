...
describe('server/email-notification-service.js iteration131', () => {
  it('returns explicit getAdminEmails failure object in notifyNewEvent when storage returns success=false', async () => {
    const explicitError = new Error('explicit-admin-failure-131');
    setupDependencies({
      adminsResult: { success: false, error: explicitError },
    });
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewEvent({ title: 'Event 131' }, 'Org');

    expect(result.success).toBe(false);
    expect(result.error).toBe(explicitError);
  });
});
