...
type LoggerMock = { error: ReturnType<typeof vi.fn>; warn: ReturnType<typeof vi.fn>; info: ReturnType<typeof vi.fn>; debug: ReturnType<typeof vi.fn> };
...
function loadDbModule(): { module: DbJsModule; neonPool: MockPool; loggerMock: LoggerMock } {
  cleanupHarness();
  process.env.DATABASE_URL = 'postgresql://user:pass@cluster.neon.tech/appdb';
  process.env.NODE_ENV = 'production';

  const stats: PoolStats = { totalCount: 10, idleCount: 4, waitingCount: 3 };
  let createdNeonPool: MockPool | null = null;

  class NeonPoolMock extends MockPool {
    public constructor(options: Record<string, unknown>) {
      super(options, stats);
      createdNeonPool = this;
    }
  }
  class PgPoolMock extends MockPool {}

  const loggerMock: LoggerMock = { error: vi.fn(), warn: vi.fn(), info: vi.fn(), debug: vi.fn() };
  ...
  setCjsMock(loggerModulePath, { logger: loggerMock });
  ...
  return { module, neonPool: createdNeonPool as MockPool, loggerMock };
}

describe('server/db.js iteration127 neon error listener callback', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    cleanupHarness();
  });

  it('logs structured error payload when neon pool emits error', () => {
    const setup = loadDbModule();
    const err = new Error('neon-pool-error');

    setup.neonPool.emit('error', err, null);

    expect(setup.loggerMock.error).toHaveBeenCalledWith(
      'CRITICAL: Database pool error',
      expect.objectContaining({
        provider: 'neon',
        message: 'neon-pool-error',
        poolStats: expect.objectContaining({ totalCount: 10, idleCount: 4, waitingCount: 3 }),
      }),
    );
  });
});
