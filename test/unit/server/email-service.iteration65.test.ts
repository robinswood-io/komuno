...
describe('server/email-service.js (iteration 65)', () => {
  beforeEach(() => {
  ...
  it('testConnection peut échouer puis réussir sur deux appels successifs', async () => {
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';

    const emailService = loadEmailService();
    await emailService.reloadConfig();
    verifyMock.mockRejectedValueOnce(new Error('first fail'));
