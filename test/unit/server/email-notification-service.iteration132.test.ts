...
describe('server/email-notification-service.js iteration132', () => {
  it('reuses cached admin emails when options are omitted', async () => {
    ...
    const primed = await emailNotificationService.getAdminEmails({ forceRefresh: true });
    const cached = await emailNotificationService.getAdminEmails();

    expect(primed.success).toBe(true);
    expect(cached.success).toBe(true);
    expect(cached.data).toEqual(['cached@example.com']);
