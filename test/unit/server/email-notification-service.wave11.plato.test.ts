...
function setupDependencies(): { storageMock: StorageMock } {
  ...
  const emailServiceMock: EmailServiceMock = {
    sendEmail: vi.fn(async () => ({ success: true, data: { messageId: 'msg-wave11' } })),
  };
...
describe('server/email-notification-service.js wave11 plato', () => {
  it('returns cached admin recipients when cache is warm and forceRefresh is not set', async () => {
    const { storageMock } = setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const warm = await emailNotificationService.getAdminEmails({ forceRefresh: true });
    const cached = await emailNotificationService.getAdminEmails();

    expect(storageMock.getAllAdmins).toHaveBeenCalledTimes(1);
    expect(warm).toEqual({ success: true, data: ['admin@example.com'] });
    expect(cached).toEqual({ success: true, data: ['admin@example.com'] });
  });
});
