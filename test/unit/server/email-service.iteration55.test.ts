...
describe('server/email-service.js (iteration 55)', () => {
  beforeEach(() => {
  ...
  it('testConnection encapsule un objet Error en message explicite', async () => {
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';

    const emailService = loadEmailService();
    await emailService.reloadConfig();
    verifyMock.mockRejectedValueOnce(new Error('transient outage'));

    const result = await emailService.testConnection();

    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.message).toBe('Test connexion échoué: Error: transient outage');
    }
  });
});
