...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => ({ success: true, data: [{ isActive: true, status: 'active', email: 'test48@example.com' }] })),
  ...
  setCjsModule(emailServicePath, {
    emailService: {
      sendEmail: vi.fn(async () => {
        throw new Error('test-send-throw-wave48');
      }),
    },
  });
  setCjsModule(emailTemplatesPath, {
    ...
    createTestEmailTemplate: vi.fn(() => ({ subject: 'test-48', html: '<p>test-48</p>' })),
  });
...
describe('email-notification-service wave48 plato', () => {
  it('testEmailConfiguration catches send throw and returns wrapped test error', async () => {
    setupDependencies();
    ...
    if (result.error instanceof Error) {
      ...
      expect(result.error.message).toContain('test-send-throw-wave48');
    } else {
