...
describe('server/email-service.js (iteration 49)', () => {
  beforeEach(() => {
  ...
  it("journalise la source \"variables d'environnement\" sans config DB", async () => {
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';
    ...
    expect(loggerMock.info).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ source: "variables d'environnement" }),
    );
  });
});
