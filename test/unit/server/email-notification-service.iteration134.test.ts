...
describe('server/email-notification-service.js iteration134', () => {
  it('short-circuits notifyNewEvent with getAdminEmails formatted error when admin normalization crashes', async () => {
    ...
    const result = await emailNotificationService.notifyNewEvent({ title: 'Event 134' }, 'Org 134');

    expect(result.success).toBe(false);
    expect(result.error).toBeInstanceOf(Error);
    expect((result.error as Error).message).toContain('Erreur récupération admins');
    expect(templatesMock.createNewEventEmailTemplate).not.toHaveBeenCalled();
