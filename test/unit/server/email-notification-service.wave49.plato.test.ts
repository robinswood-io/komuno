...
function setupDependencies(): void {
  const recruitmentFailure = new Error('recruitment-failure-wave49');
  const storageMock: StorageMock = {
    getBrandingConfig: vi.fn(async () => ({ success: true, data: {} })),
    getAllAdmins: vi.fn(async () => ({ success: true, data: [] })),
    getMemberByCjdRole: vi.fn(async () => ({ success: false, error: recruitmentFailure })),
  };
...
describe('email-notification-service wave49 plato', () => {
  it('getRecruitmentManagerEmail returns explicit role lookup failure object', async () => {
    setupDependencies();
    ...
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('recruitment-failure-wave49');
    } else {
