...
describe('server/email-notification-service.js iteration124', () => {
  it('rejects when notifyNewLoanItem sendEmail throws unexpectedly', async () => {
    setupDependencies({ sendEmailThrows: new Error('smtp-throw-loan') });
    const { emailNotificationService } = loadServiceModule();

    await expect(emailNotificationService.notifyNewLoanItem({ title: 'Loan 124' })).rejects.toThrow(
      'smtp-throw-loan',
    );
  });
});
