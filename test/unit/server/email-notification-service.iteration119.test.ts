...
describe('server/email-notification-service.js iteration119', () => {
  it('keeps existing branding unchanged when branding lookup succeeds without data payload', async () => {
    setupDependencies({
      brandingResult: { success: true },
    });
    ...
    expect(emailNotificationService.context.branding).toEqual(existingBranding);
  });
});
