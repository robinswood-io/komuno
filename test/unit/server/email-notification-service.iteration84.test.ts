...
describe('server/email-notification-service.js iteration84', () => {
  it('skips sending when no active admin recipients are available', async () => {
    const { emailServiceMock } = setupDependencies({
      adminsResult: {
        success: true,
        data: [{ isActive: false, status: 'inactive', email: 'nobody@example.com' }],
      },
    });
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewIdea({ title: 'Idea 84' });
