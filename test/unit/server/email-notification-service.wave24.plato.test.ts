...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => ({
      success: true,
      data: [{ isActive: true, status: 'active', email: 'event-admin@example.com' }],
    })),
  ...
  setCjsModule(emailServicePath, {
    emailService: {
      sendEmail: vi.fn(async () => {
        throw new Error('send-throw-wave24');
      }),
    },
  });
  setCjsModule(emailTemplatesPath, {
    createNewIdeaEmailTemplate: vi.fn(() => ({ subject: 'idea', html: '<p>idea</p>' })),
    createNewEventEmailTemplate: vi.fn(() => ({ subject: 'event-24', html: '<p>event-24</p>' })),
...
describe('email-notification-service wave24 plato', () => {
  it('notifyNewEvent propagates thrown sendEmail error as a rejected promise', async () => {
    setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    await expect(emailNotificationService.notifyNewEvent({ title: 'Event 24' }, 'Zoé')).rejects.toThrow(
      'send-throw-wave24',
    );
  });
});
