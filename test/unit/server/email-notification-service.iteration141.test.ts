...
describe('server/email-notification-service.js iteration141', () => {
  it('stringifies non-Error template crash in notifyNewEvent catch branch', async () => {
    const { templatesMock } = setupDependencies();
    templatesMock.createNewEventEmailTemplate.mockImplementation(() => {
      throw 'event-template-crash-141';
    });
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewEvent({ title: 'Event 141' }, 'Org 141');

    expect(result.success).toBe(false);
    expect(result.error).toBeInstanceOf(Error);
    expect((result.error as Error).message).toContain('event-template-crash-141');
  });
});
