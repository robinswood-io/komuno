...
function loadDbModule(): { module: DbJsModule; neonPool: MockPool } {
  cleanupHarness();
  process.env.DATABASE_URL = 'postgresql://user:pass@cluster.neon.tech/appdb';
  process.env.NODE_ENV = 'development';

  const stats: PoolStats = { totalCount: 9, idleCount: 3, waitingCount: 1 };
  ...
  return { module, neonPool: createdNeonPool as MockPool };
}

describe('server/db.js iteration124 neon connect listener callback', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    cleanupHarness();
  });

  it('logs pool connect event in development on neon provider', () => {
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const setup = loadDbModule();

    setup.neonPool.emit('connect');

    expect(consoleLogSpy).toHaveBeenCalledWith('[DB] Nouvelle connexion établie (provider: neon, pool: 9)');
  });
});
