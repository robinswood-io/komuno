...
describe('server/email-notification-service.js iteration120', () => {
  it('uses forceRefresh in testEmailConfiguration and ignores previously cached admin recipients', async () => {
    const { storageMock, emailServiceMock } = setupDependencies({
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: 'cached@example.com' }],
      },
    });

    const { emailNotificationService } = loadServiceModule();

    const priming = await emailNotificationService.getAdminEmails({ forceRefresh: true });
    expect(priming).toEqual({ success: true, data: ['cached@example.com'] });

    storageMock.getAllAdmins.mockResolvedValueOnce({
      success: true,
      data: [{ isActive: true, status: 'active', email: 'fresh@example.com' }],
    });

    await emailNotificationService.testEmailConfiguration();
