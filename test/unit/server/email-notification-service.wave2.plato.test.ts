...
function setupDependencies(): { storageMock: StorageMock; emailServiceMock: EmailServiceMock } {
  ...
  const emailServiceMock: EmailServiceMock = {
    sendEmail: vi.fn(async () => ({ success: true, data: { messageId: 'msg-plato-wave2' } })),
  };
...
describe('server/email-notification-service.js wave2 plato', () => {
  it('testEmailConfiguration fails when no active administrator email is available', async () => {
