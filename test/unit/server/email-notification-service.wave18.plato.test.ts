...
function setupDependencies(): { storageMock: StorageMock; emailServiceMock: EmailServiceMock } {
  const sendFailure = new Error('smtp-refused-wave18');
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewEventEmailTemplate: vi.fn(() => ({ subject: 'event-subject-18', html: '<p>event-18</p>' })),
...
describe('server/email-notification-service.js wave18 plato', () => {
  it('notifyNewEvent propagates sendEmail failure result after template/render success', async () => {
    ...
    const result = await emailNotificationService.notifyNewEvent(
      { title: 'Soirée réseau', description: 'Pitchs adhérents' },
      'Martin',
    );
    ...
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('smtp-refused-wave18');
    } else {
