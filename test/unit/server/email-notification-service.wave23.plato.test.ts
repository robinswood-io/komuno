...
function setupDependencies(): { storageMock: StorageMock } {
  const adminFailure = new Error('admin-list-failure-wave23');
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewEventEmailTemplate: vi.fn(() => ({ subject: 'event-23', html: '<p>event-23</p>' })),
...
describe('email-notification-service wave23 plato', () => {
  it('notifyNewEvent returns admin lookup failure object before template rendering', async () => {
    const { storageMock } = setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewEvent({ title: 'Event 23' }, 'Luc');

    expect(storageMock.getAllAdmins).toHaveBeenCalledTimes(1);
    expect(result.success).toBe(false);
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('admin-list-failure-wave23');
    } else {
