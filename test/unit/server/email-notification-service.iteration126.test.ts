...
describe('server/email-notification-service.js iteration126', () => {
  it('returns success with no send when recruitment-manager email is whitespace-only', async () => {
    const { emailServiceMock } = setupDependencies({
      recruitmentResult: { success: true, data: { email: '   ' } },
    });
