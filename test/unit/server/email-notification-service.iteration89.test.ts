...
describe('server/email-notification-service.js iteration89', () => {
  it('returns send failure when testEmailConfiguration cannot send test email', async () => {
    const { emailServiceMock } = setupDependencies({
      sendEmailResult: { success: false, error: new Error('smtp-test-failure') },
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: 'test-admin@example.com' }],
      },
    });
    ...
    expect(payload.to).toEqual(['test-admin@example.com']);
  });
});
