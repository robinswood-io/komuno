...
describe('server/email-service.js (iteration 69)', () => {
  beforeEach(() => {
  ...
  it('propage la propriété response de sendMail dans le résultat', async () => {
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      const payload = result.data as { response?: string };
      expect(payload.response).toBe('250 2.0.0 Accepted');
    }
  });
});
