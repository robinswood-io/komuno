...
describe('server/email-notification-service.js iteration82', () => {
  it('uses cached admin emails on second call without forceRefresh', async () => {
    const { storageMock } = setupDependencies({
      adminsResult: {
        ...
        data: [
          ...
          { isActive: true, status: 'active', email: 'admin@example.com' },
          { isActive: false, status: 'active', email: 'ignored@example.com' },
        ],
      },
    });
    ...
    expect(second.data).toEqual(['admin@example.com']);
