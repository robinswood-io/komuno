...
describe('server/email-notification-service.js wave5 plato', () => {
  it('getAdminEmails wraps thrown storage errors into domain error', async () => {
    ...
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('Erreur récupération admins');
      expect(result.error.message).toContain('db-read-failure-wave5');
    } else {
