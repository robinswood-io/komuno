...
function setupDependencies(): { storageMock: StorageMock } {
  ...
  const emailServiceMock: EmailServiceMock = {
    sendEmail: vi.fn(async () => ({ success: true, data: { messageId: 'msg-wave20' } })),
  };
...
describe('server/email-notification-service.js wave20 plato', () => {
  it('getRecruitmentManagerEmail returns success with null when no recruitment manager email exists', async () => {
