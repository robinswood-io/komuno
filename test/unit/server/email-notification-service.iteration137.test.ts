...
describe('server/email-notification-service.js iteration137', () => {
  it('logs success metadata when testEmailConfiguration sends test email', async () => {
    const { loggerMock } = setupDependencies({
      adminsResult: {
        success: true,
        data: [{ isActive: true, status: 'active', email: 'admin137@example.com' }],
      },
      sendEmailResult: { success: true, data: { messageId: 'msg-137' } },
    });
    ...
    expect(loggerMock.info).toHaveBeenCalledWith(
      '[Email Notifications] Test email envoyé avec succès',
      expect.objectContaining({ recipient: 'admin137@example.com', messageId: 'msg-137' }),
    );
  });
});
