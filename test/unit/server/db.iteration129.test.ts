...
describe('server/db.js iteration129 neon drizzle logger callback', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    cleanupHarness();
  });

  it('executes neon drizzle logQuery callback and logs query prefix', () => {
    ...
    logger.logQuery('select id from admins', []);

    expect(consoleLogSpy).toHaveBeenCalledWith('[DB Query] select id from admins...');
  });
});
