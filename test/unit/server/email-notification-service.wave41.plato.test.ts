...
function setupDependencies(): { emailServiceMock: EmailServiceMock } {
  const sendFailure = new Error('loan-send-failure-wave41');
  const storageMock: StorageMock = {
    getBrandingConfig: vi.fn(async () => ({ success: true, data: {} })),
    getAllAdmins: vi.fn(async () => ({ success: true, data: [{ isActive: true, status: 'active', email: 'loan41@example.com' }] })),
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewLoanItemEmailTemplate: vi.fn(() => ({ subject: 'loan-41', html: '<p>loan-41</p>' })),
...
describe('email-notification-service wave41 plato', () => {
  it('notifyNewLoanItem propagates sendEmail failure object', async () => {
    const { emailServiceMock } = setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewLoanItem({ title: 'Wave41 loan' });

    expect(emailServiceMock.sendEmail).toHaveBeenCalledTimes(1);
    expect(result.success).toBe(false);
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('loan-send-failure-wave41');
    } else {
