...
describe('server/email-notification-service.js iteration91', () => {
  it('keeps context stable when loadBranding encounters storage exception', async () => {
    setupDependencies({
      brandingThrows: new Error('branding-db-down'),
    });
    const { emailNotificationService } = loadServiceModule();

    const before = { ...emailNotificationService.context };
    await emailNotificationService.loadBranding();
    const after = emailNotificationService.context;

    expect(after.baseUrl).toBe(before.baseUrl);
    expect(after.adminDashboardUrl).toBe(before.adminDashboardUrl);
  });
});
