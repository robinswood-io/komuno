...
describe('server/email-service.js (iteration 50)', () => {
  beforeEach(() => {
  ...
  it('fallback sur SMTP_HOST env si host DB vide', async () => {
    process.env.SMTP_HOST = 'env-fallback-host.smtp.local';
    ...
    expect(createTransportMock).toHaveBeenCalledWith(
      expect.objectContaining({
        host: 'env-fallback-host.smtp.local',
        port: 2528,
        secure: false,
        auth: {
          user: 'db-user',
