...
describe('server/email-notification-service.js iteration83', () => {
  it('returns failure when getAllAdmins fails', async () => {
    setupDependencies({
      adminsResult: { success: false, error: new Error('db-admins-failure') },
    });
    ...
    expect(result.error?.message).toContain('db-admins-failure');
  });
});
