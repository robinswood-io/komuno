...
describe('http-exception.filter iteration 92', () => {
  ...
  it('retourne status/code/message pour ApiError avec code', () => {
    ...
    filter.catch(new ApiError(422, 'Une erreur interne est survenue. Réessayez ou contactez le support avec l’identifiant indiqué.', 'VALIDATION_FAILED'), createHost(request, response));
    ...
    expect(metadata.message).toBe('Une erreur interne est survenue. Réessayez ou contactez le support avec l’identifiant indiqué.');
  });
});
