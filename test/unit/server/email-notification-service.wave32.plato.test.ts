...
function setupDependencies(): { storageMock: StorageMock } {
  const storageFailure = new Error('storage-failed-wave32');
  const storageMock: StorageMock = {
    getBrandingConfig: vi.fn(async () => ({ success: true, data: {} })),
    getAllAdmins: vi.fn(async () => ({ success: false, error: storageFailure })),
...
describe('email-notification-service wave32 plato', () => {
  it('getAdminEmails returns storage failure object when admin query fails explicitly', async () => {
    ...
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('storage-failed-wave32');
    } else {
