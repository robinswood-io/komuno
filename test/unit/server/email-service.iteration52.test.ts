...
describe('server/email-service.js (iteration 52)', () => {
  beforeEach(() => {
  ...
  it('convertit le HTML complexe en texte brut normalisé quand text est absent', async () => {
    process.env.SMTP_USER = 'env-user';
    process.env.SMTP_PASSWORD = 'env-pass';
    ...
    await emailService.sendEmail({
      to: ['user@example.com'],
      subject: 'Sujet',
      html: '<div> A&nbsp;&amp;&nbsp;B <strong> C </strong> </div>',
    });
