...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => ({ success: true, data: [{ isActive: true, status: 'active', email: 'test37@example.com' }] })),
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createTestEmailTemplate: vi.fn(() => {
      throw new Error('template-test-failure-wave37');
    }),
  });
...
describe('email-notification-service wave37 plato', () => {
  it('testEmailConfiguration catches template exception and returns wrapped test error', async () => {
    setupDependencies();
    ...
    if (result.error instanceof Error) {
      ...
      expect(result.error.message).toContain('template-test-failure-wave37');
    } else {
