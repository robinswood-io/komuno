...
describe('server/email-service.js (iteration 68)', () => {
  beforeEach(() => {
  ...
  it('fallback sur auth.user quand fromEmail devient vide après init', async () => {
    ...
    expect(sendMailMock).toHaveBeenCalledWith(
      expect.objectContaining({
        from: '"Env Sender" <env-user@example.com>',
      }),
    );
  });
});
