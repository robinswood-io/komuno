...
function setupDependencies(): { storageMock: StorageMock; emailServiceMock: EmailServiceMock } {
  ...
  const emailServiceMock: EmailServiceMock = {
    sendEmail: vi.fn(async () => ({ success: true, data: { messageId: 'msg-wave17' } })),
  };
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewMemberProposalEmailTemplate: vi.fn(() => ({ subject: 'member-subject-17', html: '<p>member-17</p>' })),
...
describe('server/email-notification-service.js wave17 plato', () => {
  it('notifyNewMemberProposal targets only recruitment manager when role email exists', async () => {
    ...
    const result = await emailNotificationService.notifyNewMemberProposal({
      ...
      phone: '0600112233',
      motivation: 'Créer du lien local',
    });
    ...
    expect(payload).toEqual({
      to: ['recruit@example.com'],
      subject: 'member-subject-17',
      html: '<p>member-17</p>',
    });
