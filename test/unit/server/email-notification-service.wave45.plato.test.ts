...
function setupDependencies(): void {
  const storageMock: StorageMock = {
    ...
    getAllAdmins: vi.fn(async () => ({ success: true, data: [{ isActive: true, status: 'active', email: 'event45@example.com' }] })),
  ...
  setCjsModule(emailTemplatesPath, {
    ...
    createNewEventEmailTemplate: vi.fn(() => {
      throw new Error('event-template-failure-wave45');
    }),
...
describe('email-notification-service wave45 plato', () => {
  it('notifyNewEvent catches template error and returns wrapped event message', async () => {
    setupDependencies();
    const { emailNotificationService } = loadServiceModule();

    const result = await emailNotificationService.notifyNewEvent({ title: 'Event 45' }, 'Iris');

    expect(result.success).toBe(false);
    if (result.error instanceof Error) {
      expect(result.error.message).toContain('Erreur notification événement');
      expect(result.error.message).toContain('event-template-failure-wave45');
    } else {
