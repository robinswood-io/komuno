...
function loadDbModule(): { module: DbJsModule; pgPool: MockPool; loggerMock: LoggerMock } {
  ...
  const stats: PoolStats = { totalCount: 6, idleCount: 2, waitingCount: 0 };
...
describe('server/db.js iteration122 standard connect listener callback', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    cleanupHarness();
  });

  it('logs pool connect event in development on standard provider', () => {
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const setup = loadDbModule();

    setup.pgPool.emit('connect');

    expect(consoleLogSpy).toHaveBeenCalledWith('[DB] Nouvelle connexion établie (provider: standard, pool: 6)');
  });
});
