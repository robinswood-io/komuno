...
function setupDependencies(): { storageMock: StorageMock; emailServiceMock: EmailServiceMock } {
  ...
  const emailServiceMock: EmailServiceMock = {
    sendEmail: vi.fn(async () => ({ success: true, data: { messageId: 'msg-plato-wave3' } })),
  };
  ...
  const templatesMock: TemplatesMock = {
    ...
    createNewLoanItemEmailTemplate: vi.fn(() => {
      throw new Error('template-boom-wave3');
    }),
...
describe('server/email-notification-service.js wave3 plato', () => {
  it('notifyNewLoanItem catches template failures and returns a wrapped domain error', async () => {
    ...
    const result = await emailNotificationService.notifyNewLoanItem({
      title: 'Caméra 4K',
      description: 'Prêt matériel vidéo',
    });
    ...
    if (result.error instanceof Error) {
      ...
      expect(result.error.message).toContain('template-boom-wave3');
    } else {
