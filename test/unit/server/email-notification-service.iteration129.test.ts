...
describe('server/email-notification-service.js iteration129', () => {
  it('hydrates branding from direct object payload when config field is absent', async () => {
    setupDependencies({
      brandingResult: {
        success: true,
        data: {
          colors: { primary: '#123abc' },
          app: { shortName: 'K129', name: 'Komuno 129' },
          organization: { fullName: 'Org 129', name: 'Org Name 129' },
        },
      },
    });
