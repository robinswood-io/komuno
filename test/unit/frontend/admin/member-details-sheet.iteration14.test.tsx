...
describe('MemberDetailsSheet - iteration14', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resetState();
  ...
  it('enregistre une interaction avec durée et notes', () => {
    render(<MemberDetailsSheet email="john@example.com" open onClose={vi.fn()} />);
  ...
  it('affiche le badge "Inactif" pour un membre inactif', () => {
    state.detailsQuery = {
      data: {
        success: true,
        data: {
    ...
    expect(screen.getByTestId('member-status-badge').textContent).toContain('Inactif');
  });

  it('ferme le dialog d’ajout d’interaction via le bouton Annuler', () => {
    render(<MemberDetailsSheet email="john@example.com" open onClose={vi.fn()} />);
    ...
    expect(screen.queryByText('Nouvelle interaction')).toBeNull();
  });
});
