...
const getRelationBadgeColor = (type: string) => {
  const colors: Record<string, string> = {
    sponsor: 'bg-info-light text-info-dark border-info',
...
/**
 * Composant Tableau pour gérer les relations (CRUD)
 */
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {filteredRelations.length} relation(s) trouvée(s)
            {filteredRelations.length !== relations.length && ` (${relations.length} au total)`}
          </CardDescription>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {RELATION_TYPES.map((type) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => setRelationTypeFilter(type.value)}
                >
